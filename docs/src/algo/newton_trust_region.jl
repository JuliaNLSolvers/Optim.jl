...
Newton's method optimizes a quadratic approximation to a function.  When a function is well approximated by a quadratic (for example, near an optimum), Newton's method converges very quickly by exploiting the second-order information in the Hessian matrix.  However, when the function is not well-approximated by a quadratic, either because the starting point is far from the optimum or the function has a more irregular shape, Newton steps can be erratic, large steps to distant, irrelevant areas of the space.

Trust region methods use second-order information but restrict the step to be within a "trust region" where the function is believed to be approximately quadratic.  At step $k$, a trust region method chooses a step $p$ to minimize a quadratic approximation to the objective such that the step size is no larger than a given trust region size, $\Delta_k$.

$\underset{p\in\mathbb{R}^n}\min m_k(p) = f_k + g_k^T p + \frac{1}{2}p^T B_k p \quad\textrm{such that } ||p||\le \Delta_k$

Here, $p$ is the step to take at iteration $k$, so that $x_{k+1} = x_k + p$.   In $m_k(p)$, $f_k = f(x_k)$ is the value at the previous location, $g_k=\nabla f(x_k)$ is the gradient at the previous location, $B_k = \nabla^2 f(x_k)$ is the Hessian matrix at the previous iterate, and $||\cdot||$ is the Euclidian norm.

If the trust region size, $\Delta_k$, is large enough that the minimizer of the quadratic approximation $m_k(p)$ has $||p|| \le \Delta_k$, then the step is the same as an ordinary Newton step.  However, if the minimizer lies outside the trust region, then the minimizer will occur on the boundary, and the step will have $||p|| = \Delta_k$.  It turns out that when the Cholesky decomposition of $B_k$ can be computed, the optimal $p$ can be found numerically with relative ease.  (See [1], section 4.3)  This is the method currently used in ```Optim.jl```.

It makes sense to adapt the trust region size, $\Delta_k$, as we move through the space and assess the quality of the quadratic fit.  This adaptation is controlled by the parameters $\eta$, $\rho_{lower}$, and $\rho_{upper}$, which are parameters to the ```NewtonTrustRegion``` optimization module.  For each step, we calculate

$\rho_k := \frac{f(x_{k+1}) - f(x_k)}{m_k}$

If $\rho_k \approx 1$, then our quadratic approximation is reasonable.  If  $p$ was on the boundary and $\rho_k > \rho_{upper}$, then perhaps we can gain by taking larger steps.  In this case, for the next iteration we grow the trust region geometrically up to a maximum of $\hat\Delta$:
...
Finally, we only accept a point if its decrease is appreciable compared to the quadratic approximation $\rho_k > \eta$ where $\eta$ is chosen to be less than $\rho_{lower}$ so that we will shrink the trust region whenever we reject a step.
