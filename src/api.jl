...
function x_lower_trace(r::UnivariateOptimizationResults)
    tr = trace(r)
    !haskey(tr[1].metadata, "x_lower") && error(
        "Trace does not contain x. To get a trace of x, run optimize() with extended_trace = true",
    )
    [state.metadata["x_lower"] for state in tr]
end
x_lower_trace(r::MultivariateOptimizationResults) =
    error("x_lower_trace is not implemented for $(summary(r)).")
function x_upper_trace(r::UnivariateOptimizationResults)
    tr = trace(r)
    !haskey(tr[1].metadata, "x_upper") && error(
        "Trace does not contain x. To get a trace of x, run optimize() with extended_trace = true",
    )
...
function centroid_trace(r::MultivariateOptimizationResults)
    ...
    !haskey(tr[1].metadata, "centroid") && error(
        "Trace does not contain centroid. To get a trace of the centroid, run optimize() with extended_trace = true",
    )
...
function simplex_trace(r::MultivariateOptimizationResults)
    ...
    !haskey(tr[1].metadata, "simplex") && error(
        "Trace does not contain simplex. To get a trace of the simplex, run optimize() with trace_simplex = true",
    )
...
upper_bound(r::OptimizationResults) =
    error("upper_bound is not implemented for $(summary(r)).")
...
time_limit(r::OptimizationResults) =
    error("time_limit is not implemented for $(summary(r)).")
