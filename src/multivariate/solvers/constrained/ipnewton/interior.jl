...
Base.isempty(bstate::BarrierStateVars) =
    isempty(bstate.slack_x) &
    isempty(bstate.slack_c) &
    isempty(bstate.λxE) &
    isempty(bstate.λcE)
...
Base.hash(b::BarrierStateVars, u::UInt) = hash(
    b.λcE,
...
function ls_update!(
    out::BarrierStateVars,
    base::BarrierStateVars,
    step::BarrierStateVars,
    αs::NTuple{4,Number},
)
...
constraint jacobian must be set in `state.g_x`, `state.c`, and `state.J`
...
hessian_projections(Hinfo::Nothing, Pperpg::AbstractVector{T}) where {T} =
    convert(T, Inf), zero(T)
...
function _lagrangian_lineslope(αs, d, constraints, state)
    ...
    f_x, g_x = value_gradient!(d, state.x_ls)
    ...
    slopeα = slopealpha(state.s, state.g_x, bstep, bgrad)
    f_x, L, ev, slopeα
end
...
function lagrangian_lineslope!(
    ...
    constraints,
    state,
    method::IPOptimizer{typeof(backtrack_constrained_grad)},
)
    # For backtrack_constrained, the last evaluation is the one we
    # keep, so it's safe to store the results in state
...
barrier_grad!(bgrad, bounds::ConstraintBounds, x, bstate, μ) = barrier_grad!(
    bgrad.slack_x,
    bgrad.slack_c,
    bounds,
    x,
...
function equality_violation(f, s, v, ineq, σ, b, λ)
    ret = f(
        zero(eltype(λ)) *
        (zero(eltype(s)) - zero(eltype(σ)) * (zero(eltype(v)) - zero(eltype(b)))),
    )
