@testset "RMSprop" begin
    run_optim_tests(RMSprop(),
    ...
    results = Optim.optimize(d, initial_x, RMSprop())
    @test_throws ErrorException Optim.x_trace(results)
    @test Optim.g_converged(results)
    @test norm(Optim.minimizer(results) - [5.0]) < 0.01
    @test summary(results) == "Root Mean Square propagation"
    ...
    results = Optim.optimize(d, [1.0, 1.0], RMSprop())
    @test_throws ErrorException Optim.x_trace(results)
    @test Optim.g_converged(results)
